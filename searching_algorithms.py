...
# Iterative stochastic search in latent space for adversaries
def iterative_search(gen_fn, inv_fn, cla_fn, x, y, y_t=None, z=None,
    ...
    # l corresponds to r in the paper, the lower bound
    # step is delta_r
    h = l + step
    ...
    # getting the dense representation of x, using the Inverter
    if z is None:
        z = inv_fn(x)
        z = z.detach().numpy()
    ...
    #print(type(z))
    #print(type(z_adv))
    delta_z_final = ((z - z_adv) ** 2).sum()

    adversary = {'x': x, 'y': y, 'z': z,
                 'x_adv': x_adv, 'y_adv': y_adv, 'z_adv': z_adv, 'd_adv': d_adv, "delta_z": delta_z_final}
...
def recursive_search(gen_fn, inv_fn, cla_fn, x, y, y_t=None, z=None,
    ...
    while counter <= stop and h > 1e-4:
        ...
        delta_z = np.multiply(delta_z, d_norm)
        z_tilde = z + delta_z
        x_tilde = gen_fn(torch.from_numpy(z_tilde.astype(np.float32)))
        y_tilde = cla_fn(x_tilde.detach().numpy())
